/**
 * AuraClient — Web version of the Connectome HTTP client
    ...
    try {
      ...
      if (e?.response?.status === 404) {
        ...
        for (let i = 0; i < count; i++) {
          try {
            results.push(await this.getNextScreen(context, goalId, domain, feedMode));
          } catch { break; }
        }
        return results;
      }
...
export const AuraClient = new AuraClientClass();
